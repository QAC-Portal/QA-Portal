...
export class ViewCvService {
  ...
  getCvForId(id: string): Observable<ICvModel> {
    return this.qaHttp.get<ICvModel>({ ref: 'GET_CV_FOR_ID_URL', params: { id: id.toString() }}).pipe(
      take(1)
    );
  }

  getSkillsForTrainee(): Observable<any> {
    return this.qaHttp.get<any>({ ref: 'GET_SKILLS_FOR_TRAINEE_URL'}).pipe(
      take(1)
    );
  }

  getCurrentCvForTrainee(): Observable<ICvModel> {
    return this.qaHttp.get<ICvModel>({ ref: 'GET_CURRENT_CV_URL'}).pipe(
      take(1)
    );
  }
  ...
  createCv(cv: ICvModel): Observable<ICvModel> {
    return this.qaHttp.post<ICvModel>({ ref: 'SAVE_CV_DATA_URL'}, cv).pipe(
      take(1)
    );
  }
  ...
  updateCv(cv: ICvModel): Observable<ICvModel> {
    return this.qaHttp.put<ICvModel>({ ref: 'SAVE_CV_DATA_URL'}, cv).pipe(
      take(1)
    );
  }
}
